...
def main():
    ...
    ideal_trials=4000; recovered=0; hist=Counter()
    for _ in range(ideal_trials):
        share=rng.randrange(2); J=rng.randrange(8)
        outs=[]
        for p in assigns:
            outs.append(share if malformed[p] != J else 1-share)
        ...
        for Sidx in itertools.combinations(range(8),s):
            ...
            for share in (0,1):
                for badbits in itertools.product((0,1),repeat=s):
                    badmap=dict(zip(Sidx,badbits)); outs=[]
                    for p in assigns:
                        j=malformed[p]
    ...
    for _ in range(fixtures):
        ...
        J=rng.randrange(8)
        for idx in block_indices[J]: E[idx]+=17*rng.choice((-1,0,1))
        ...
        for p in assigns:
            for q in assigns:
                ...
                assert lhs==rhs; pairwise_checks+=1
    ...
    for b,(vs,rows) in enumerate(block_rows):
        FT=[[1,*r] for r in rows]
        base_rank=modrank(FT,101); assert base_rank==4
        inv=0
        for eta in itertools.product((-1,1),repeat=7):
            aug=[FT[i]+[eta[i]] for i in range(7)]
            if modrank(aug,101)==base_rank: inv+=1
    ...
    for s in (1,4,7,8):
        ...
        for _ in range(TRIALS):
            S=set(rng.sample(range(8),s)); block_noise={}
            for b in S:
                rows=block_rows[b][1]
