...
/// Compute product-key anchor using attestation commitments and instance masks
pub fn compute_product_key_anchor(
    ...
    if c1_bytes_list.len() != d1_instance_masks.len() {
        return Err("C1 and instance mask counts mismatch".to_string());
    }
    if c2_bytes_list.len() != d2_instance_masks.len() {
        return Err("C2 and instance mask counts mismatch".to_string());
    }

    let mut product = Fq12::one();

    for (c1_bytes, d1_bytes) in c1_bytes_list.iter().zip(d1_instance_masks.iter()) {
        let c1 = Com1::<Bls12_381>::deserialize_compressed(c1_bytes.as_slice())
            .map_err(|e| format!("C1 deser: {:?}", e))?;
        let d1 = Com2::<Bls12_381>::deserialize_compressed(d1_bytes.as_slice())
            .map_err(|e| format!("D1 deser: {:?}", e))?;

        let PairingOutput(p0) = Bls12_381::pairing(c1.0, d1.0);
        let PairingOutput(p1) = Bls12_381::pairing(c1.1, d1.1);
        product *= p0 * p1;
    }

    for (d2_bytes, c2_bytes) in d2_instance_masks.iter().zip(c2_bytes_list.iter()) {
        let d2 = Com1::<Bls12_381>::deserialize_compressed(d2_bytes.as_slice())
            .map_err(|e| format!("D2 deser: {:?}", e))?;
        let c2 = Com2::<Bls12_381>::deserialize_compressed(c2_bytes.as_slice())
            .map_err(|e| format!("C2 deser: {:?}", e))?;

        let PairingOutput(p0) = Bls12_381::pairing(d2.0, c2.0);
        let PairingOutput(p1) = Bls12_381::pairing(d2.1, c2.1);
        product *= p0 * p1;
    }
    ...
    product
        ...
        .map_err(|e| format!("GT serialize: {:?}", e))?;
