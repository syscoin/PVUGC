...
pub struct GrothSahaiCommitments {
    crs: CRS<Bls12_381>,
    /// Dual CRS elements required for five-bucket anchor evaluation
    u_duals: Vec<Com2<Bls12_381>>, // Lives in G2
    v_duals: Vec<Com1<Bls12_381>>, // Lives in G1
}
...
impl GrothSahaiCommitments {
    ...
    pub fn new(
        crs: CRS<Bls12_381>,
        u_duals: Vec<Com2<Bls12_381>>,
        v_duals: Vec<Com1<Bls12_381>>,
    ) -> Self {
        Self {
            crs,
            u_duals,
    ...
    pub fn get_dual_elements(&self) -> (Vec<(G2Affine, G2Affine)>, Vec<(G1Affine, G1Affine)>) {
        let u_dual_pairs: Vec<(G2Affine, G2Affine)> =
            self.u_duals.iter().map(|c| (c.0, c.1)).collect();

        let v_dual_pairs: Vec<(G1Affine, G1Affine)> =
            self.v_duals.iter().map(|c| (c.0, c.1)).collect();

        (u_dual_pairs, v_dual_pairs)
    }

    /// Borrow raw dual commitment elements
    pub fn duals(&self) -> (&[Com2<Bls12_381>], &[Com1<Bls12_381>]) {
        (&self.u_duals, &self.v_duals)
    }
