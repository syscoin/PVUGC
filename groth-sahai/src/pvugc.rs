...
pub fn pvugc_arm<E: Pairing>(bases: &RankDecompPpeBases<E>, rho: &E::ScalarField) -> ArmedBases<E> {
    ArmedBases {
...
pub fn pvugc_decap<E: Pairing>(
    ...
    if !attestation.equ_proofs.is_empty() {
        for (theta, w) in attestation.equ_proofs[0]
            .theta
            .iter()
            .zip(armed_bases.DP.iter())
        {
    ...
    if !attestation.equ_proofs.is_empty() {
        for (z, pi) in armed_bases
            .DQ
            .iter()
            .zip(attestation.equ_proofs[0].pi.iter())
        {
            M += E::pairing(*z, pi.0);
            M += E::pairing(*z, pi.1);
        }
...
mod tests {
    ...
    fn test_pvugc_3x3_identity_with_real_groth16() {
        ...
        let ic = (vk.gamma_abc_g1[0].into_group() + vk.gamma_abc_g1[1].into_group() * y_val)
            .into_affine();
        ...
        println!(
            "   target_direct == target_crs? {}",
            direct_lhs == target_crs
        );
        ...
        println!(
            "  U_rand.len()={}, U_var.len()={}",
            bases_full.U_rand.len(),
            bases_full.U_var.len()
        );
        println!(
            "  V_rand.len()={}, V_var.len()={}",
            bases_full.V_rand.len(),
            bases_full.V_var.len()
        );
        ...
        assert_eq!(
            m_extracted, target,
            "Extracted M should equal target e(α,β)"
        );
        ...
        println!(
            "   K (KEM key) extracted successfully? {}",
            k_extracted == k_expected
        );
    ...
    fn test_pvugc_block_based_2x2() {
        ...
        println!(
            "  U_rand.len()={}, U_var.len()={}",
            bases_new.U_rand.len(),
            bases_new.U_var.len()
        );
        println!(
            "  V_rand.len()={}, V_var.len()={}",
            bases_new.V_rand.len(),
            bases_new.V_var.len()
        );
        ...
        println!(
            "\nBlock-based verifier: {}",
            if verifies { "PASS" } else { "FAIL" }
        );
