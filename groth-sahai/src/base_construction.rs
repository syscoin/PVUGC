...
impl<E: Pairing> RankDecompPpeBases<E> {
    ...
    pub fn build(crs: &CRS<E>, ppe: &PPE<E>, decomp: &RankDecomp<E::ScalarField>) -> Self {
        ...
        let is_pure_bilinear = ppe.a_consts.iter().all(|a| a.is_zero()) 
                            && ppe.b_consts.iter().all(|b| b.is_zero());
...
mod tests {
    ...
    fn test_W_bases_per_rank_component() {
        ...
        for (a, w_a) in W.iter().enumerate() {
            assert_ne!(*w_a, <F as Pairing>::G2Affine::zero(), "W[{}] should be non-zero", a);
        }
    ...
    fn test_build_P_slots_dimensions() {
        ...
        assert_eq!(P.len(), decomp.rank, "Should have one P slot per rank component");
    ...
    fn test_P_slots_zero_randomizers() {
        ...
        for (a, p_a) in P.iter().enumerate() {
            assert_eq!(p_a.0, <F as Pairing>::G1Affine::zero(), "P[{}].0 should be zero", a);
            assert_eq!(p_a.1, <F as Pairing>::G1Affine::zero(), "P[{}].1 should be zero", a);
        }
    ...
    fn test_Q_slots_zero_randomizers() {
        ...
        for (j, q_j) in Q.iter().enumerate() {
            assert_eq!(q_j.0, <F as Pairing>::G2Affine::zero(), "Q[{}].0 should be zero", j);
            assert_eq!(q_j.1, <F as Pairing>::G2Affine::zero(), "Q[{}].1 should be zero", j);
        }
    ...
    fn test_proof_agnostic_randomizer_cancellation() {
        ...
        for i in 0..m {
            ...
            C1_1.push(crate::data_structures::Com1(c1_0.into_affine(), c1_1.into_affine()));
            ...
            C1_2.push(crate::data_structures::Com1(c2_0.into_affine(), c2_1.into_affine()));
        }
        ...
        for j in 0..n {
            ...
            C2_1.push(crate::data_structures::Com2(c1_0.into_affine(), c1_1.into_affine()));
            ...
            C2_2.push(crate::data_structures::Com2(c2_0.into_affine(), c2_1.into_affine()));
        }
        ...
        assert_ne!(C1_1[0], C1_2[0], "Commitments should differ with different randomizers");
        assert_ne!(P1[0], P2[0], "P slots should differ with different randomizers");
        ...
        assert_eq!(M1, M2, "FAIL: PROOF-AGNOSTIC PROPERTY FAILED: Randomizer cancellation did not work!");
