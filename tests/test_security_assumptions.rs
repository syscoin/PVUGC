...
fn test_determinism_different_proofs_same_statement() {
    ...
    assert!(ppe.verify_rank_decomp(&attestation1, &crs), "Attestation 1 should verify");
    assert!(ppe.verify_rank_decomp(&attestation2, &crs), "Attestation 2 should verify");
    ...
    assert_eq!(k1, k2, "Different proofs for same statement must produce same K");
...
fn test_wrong_public_input_rejection() {
    ...
    let ppe1 = PPE { gamma: gamma1.clone(), a_consts: a_consts.clone(), b_consts: b_consts.clone(), target: target1 };
    ...
    let ppe2 = PPE { gamma: gamma2, a_consts, b_consts, target: target2 };
...
fn test_identity_element_protection() {
    ...
    assert!(ppe.verify_rank_decomp(&attestation, &crs), "Attestation should verify");
    
    let k = pvugc_decap(&attestation, &armed_bases);
    let expected = target * rho;
    
    assert_eq!(k, expected, "K should equal target^rho even with zeros in gamma");
}
...
fn test_commitment_binding() {
    ...
    let ppe1 = PPE { gamma: gamma.clone(), a_consts: a_consts.clone(), b_consts: b_consts.clone(), target: target1 };
    let ppe2 = PPE { gamma, a_consts, b_consts, target: target2 };
    ...
    let attestation1 = ppe1.commit_and_prove_rank_decomp(&vec![x_var1], &vec![y_var], &crs, &mut rng);
    let attestation2 = ppe2.commit_and_prove_rank_decomp(&vec![x_var2], &vec![y_var], &crs, &mut rng);
    ...
    assert_ne!(k1, k2, "Different committed values must produce different K (binding property)");
...
fn test_randomness_independence() {
    ...
    for _ in 0..5 {
        let attestation = ppe.commit_and_prove_rank_decomp(&x_vars, &y_vars, &crs, &mut rng);
        assert!(ppe.verify_rank_decomp(&attestation, &crs), "All attestations should verify");
